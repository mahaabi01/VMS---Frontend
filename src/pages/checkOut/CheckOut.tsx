...
const CheckOut = () => {
  ...
  const total = items.reduce((total,item)=>item.Product.productPrice * setItems.quantity + total, 0)
                    ...
                    items.length > 0 ? items.map((item)=>{
                      return(
                        <div className="flex items-start gap-4" key={item.id}>
                          < className="w-32 h-28 max-lg:w-24 max-lg:h-24 flex p-3 shrink-0 bg-gray-200 rounded-md">
